-- Minimal data for MVP authentication testing with duplicate prevention
-- Create a demo user for testing (password: "demo123")

DO $$
BEGIN
    -- Check if demo user already exists
    IF NOT EXISTS (SELECT 1 FROM users WHERE username = 'demo_user' LIMIT 1) THEN
    ...
    ELSE
        RAISE NOTICE 'Demo user already exists, skipping creation to prevent duplicates';
    END IF;
END
$$;
